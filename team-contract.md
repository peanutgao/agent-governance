...
**当前所有禁改区 owner 均为 `@peanutgao` 一人**（2026-08-20 决定暂不设备 owner）。由此产生的实际状态：他本人对禁改区的改动无人可批（GitHub 不允许自审），只能自行合并；禁改区对他不构成门禁，只对其他成员生效。团队增员后再评估是否配备 owner。
...
7. **逐条澄清，一次只问一个**：业务逻辑不明确、需求有多个合理方案或进入 grilling 时，一次只问一个问题，据上一个答复追问下一问；禁止一次抛出多个问题让用户选择或回答
