...
Use `actions/checkout@v5` with `fetch-depth: 0`, then run the repository-local script. For pull requests, check `${{ github.event.pull_request.base.sha }}..${{ github.event.pull_request.head.sha }}`. For pushes, check `${{ github.event.before }}..${{ github.sha }}`; when `before` is all zero, read the commit IDs from `$GITHUB_EVENT_PATH` with `jq` and check every pushed commit individually, including the root-commit case.
